...
> 💡 **Architecture Note for Judges:** Our backend runs on a **100% Serverless Architecture** (AWS Lambda, API Gateway, DynamoDB On-Demand) deployed via AWS CDK. Frontends are hosted on **Cloudflare Pages** for maximum reliability and global edge performance. The entire platform scales to zero and costs practically $0 when idle.
