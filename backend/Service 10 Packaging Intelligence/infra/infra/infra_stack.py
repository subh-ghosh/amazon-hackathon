...
class InfraStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        # 3. ECS Cluster
        cluster = ecs.Cluster(self, "PackagingServiceCluster", vpc=vpc)

        # 4. Fargate Service
        packaging_service = ecs_patterns.ApplicationLoadBalancedFargateService(self, "PackagingEngineService",
            cluster=cluster,
            cpu=256,
        ...
        packaging_service.target_group.configure_health_check(
            path="/health",
        ...
        # 7. IAM Permissions
        bus.grant_put_events_to(packaging_service.task_definition.task_role)

        # 8. Output
        CfnOutput(self, "PackagingServiceURL", value=f"http://{packaging_service.load_balancer.load_balancer_dns_name}")
