...
BASE_URL = "http://Circul-Logis-hgIeVqBbAk0h-362510022.us-east-1.elb.amazonaws.com"
results = []
...
def t_val_missing():
    p = base_payload.copy()
    del p["returnId"]
    r = requests.post(f"{BASE_URL}/api/v1/logistics/optimize", json=p)
...
def t_val_extra():
    ...
    r = requests.post(f"{BASE_URL}/api/v1/logistics/optimize", json=p)
    return r.status_code == 422 or (False, f"Returned {r.status_code}")
def t_val_invalid_coord():
    p = base_payload.copy()
    p["warehouses"][0]["distanceKm"] = -10.0
    r = requests.post(f"{BASE_URL}/api/v1/logistics/optimize", json=p)
    return r.status_code == 422 or (False, f"Returned {r.status_code}")
def t_val_invalid_warehouse():
    p = base_payload.copy()
    p["warehouses"] = []
    r = requests.post(f"{BASE_URL}/api/v1/logistics/optimize", json=p)
    return r.status_code == 422 or (False, f"Returned {r.status_code}")
...
run_test("Validation", "Invalid coordinates", t_val_invalid_coord)
run_test("Validation", "Invalid warehouse data", t_val_invalid_warehouse)

# 3. Biz Logic
def t_biz_run():
    r = requests.post(f"{BASE_URL}/api/v1/logistics/optimize", json=base_payload)
    if r.status_code != 200: return (False, f"Returned {r.status_code}")
    data = r.json()
    if data["recommendedWarehouse"] != "WH-1": return (False, "Did not pick lowest cost route")
    return True

run_test("Business Logic", "Optimize Route", t_biz_run)
